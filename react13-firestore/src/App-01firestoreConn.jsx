...
function App() {
  ...
  const addMessage = async () => {
    ...
    하위 데이터는 JS 객체 형식으로 작성하면 된다. 테이블처럼 정형화된 것이 
    아니므로 원하는대로 객체에 정보를 추가할 수 있다.

    문서추가를 위해 setDoc(도큐먼트정보, 추가할데이터) 와 같은 형식으로 실행
    */
    await setDoc(doc(firestore,'Korea','Gwangmyeong'),{
      gu: 'none',
      dong: '광명동',
      hotPlace: '우리집',
    });
  ...
  const getMessage = async () => {
    ...
    const docRef = doc(firestore, 'Korea','Gwangmyeong');
    // 참조를 통해 도큐먼트를 얻어온다.
