...
function ChatMessage() {
  // 쿼리 스트링으로 전달된 파라미터를 조작할때 사용하는 라우터 훅
  ...
  */
  ...
  useEffect(()=>{
    // 리스너 생성. 새로운 대화 내역이 확인되는 즉시 이벤트가 발생된다.
    onValue(dbRef,(snapshot)=>{
      ...
      snapshot.forEach((childSnapshot) => {
        ...
        if(childData.id===userId){
          /*
          데이터베이스에 등록된 데이터의 아이디와 대화창에 접속한 사용자의
          아이디가 일치하면, 본인이므로 '우측'으로 정렬한다.
          */
          ...
          // 상대방이 보낸 메세지는 좌측이로 정렬한다.
          showDiv.push(<div><div className='yourMsg'>{childData.message}</div></div>);
        }
        prevId = childData.id;
      });
