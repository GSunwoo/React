...
: 함수형 컴포넌트에서 LifeCycle(수명주기)를 사용하기 위한 훅으로
  ...
  등과 같은 작업을 수행한다.
  컴포넌트가 1차렌더링을 마친 후 실행할 코드를 정의할때 사용된다.
  즉 화면에 기본UI(밑그림)가 그려진 후 데이터를 얻어와서 채우는 방식으로 
  동작한다.
 */
...
function LifeGood (props) {
  ...
  있다면 이 부분에 작성한다.
   */
  console.log('#Life', 'LifeFood==>1. 컴포넌트 실행(함수 호출)');
  ...
  1. 2개의 버튼을 누를 때마다 useEffect가 실행된다. 즉 리렌더링이 될때마다 실행된다.
  ...
  return(<>
    ...
    {/* 버튼을 누를때마다 난수를 생성하거나, 카운트를 증가시켜 스테이트를 변경한다.
        이 변경은 새로운 렌더링으로 이어진다. */}
