...
function GlobalTop (props) {
  ...
  useEffect(function(){
    ...
    차후 useEffect가 실행되면 json 파일의 내용을 읽어와서 값을 변경한다.
     */
    // 내부에 있는 json파일을 get방식으로 요청한다.
    fetch('./jsxon/myData.json')
      .then((result)=>{
        /** 
      ...
      return ()=>{
         ...
         */
        console.log('#Life', 'LifeGood==> 4. uesEffect실행2');
      }
  }, []);
/**
 ...
 */
  ...
  for문은 실행되지 않는다. 따라서 페이지의 틀만 만들어진 상태로 렌더링된다.
   */
  for(var i=0; i<myList.length ; i++){
    ...
    listTag.push(
      <li key={data.id}>
        {/* data-id 속성에 설정된 값은 Event객체의 target속성 하위의 dataset.id를 통해
        읽어올 수 있다. 이 속성에 게시물의 일련번호인 num을 설정하고 있다. */}
...
function App() {   
  ...
  return (<>
    <h2>React - 내부서버통신</h2>    
    {/* 클릭시 내부에 저장된 dto.json 파일을 get방식으로 요청한 후 콜백데이터를
    받아오는 기능의 함수를 프롭스로 전달한다.
    자식 컴포넌트는 이 함수를 호출할때 게시물의 일련번호를 인수로 전달한다. */}
    <GlobalTop myLinkClick={(num)=>{
      console.log('클릭', num);
      fetch('./jsxon/dto'+num+'.json')
      .then((result)=>{
        return result.json();
      })
      .then((json)=>{
        console.log('결과', json);
        // JSON파일의 내용을 통해 스테이트를 변경하고 렌더링한다.
        setMyResult(json);
      });
    }}></GlobalTop>
