...
function App() {
  ...
  Step1 : 일반적인 화살표 함수 선언
          ...
          그때마다 새로운 참조값을 할당받게 된다. 즉 참조값이 계속 바뀌므로
  ...
  */
  ...
  Step2 : 함수에 useCallback을 적용하여 렌더링 시 딱 한번만 함수를
          메모이제이션한다. 하지만 의존성배열에 빈값을 주어 딱 한번만
          실행되므로 스테이트의 변경을 감지하지 못한다. 최초 실행시의
          초기값 0만 출력된다.
  Step3 : countNumber가 변경될때마다 새롭게 메모이제이션 되므로 변경된
          스테이트를 감지할 수 있다. 단 randomNumber의 변화는 감지하지
          못하므로 다른 값이 출력될 수 있다.
  */
  ...
  return (<>
    ...
    <input type='number' value={countNumber} onChange={(e)=>{
