...
function App() {
  ...
  */
  ...
  Step2 : 상수를 객체로 변경
      ...
      따라서 useEffect()가 지속으로 호출된다. 즉 불필요한 실행이 발생된다.
  */
  ...
  */
  ...
  Step2 : 객체형으로 변경하면 App컴포넌트가 렌더링될 때마다 새로운 참조값이
          할당되므로 값이 변경된 것으로 인식하여 useEffect가 지속으로 호출되는
          문제가 생긴다. 즉 애플리케이션의 성능이 저하될 수 있다.

  Step3 : App컴포넌트가 렌더링될 때마다 지속적으로 참조값이 변경되는 부분을
          차단하기 위해 useMemo를 통해 Memoization한 값을 사용하도록 코드를 수정하면
          switchMode가 변경될 때만 useEffect를 재실행한다.
  */
  useEffect(()=>{
