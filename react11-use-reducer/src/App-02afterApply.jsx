...
/*
useReducer
: useState와 유사하게 상태를 관리한다. 단 여러개의 하위값을 가진 스테이트를
  관리할때 유용하다. 또한 컴포넌트에서 상태를 업데이트하는 로직을 분리할 수
  있다.
  형식]
    Dispatch(Action) ==> Reducer(prevState, Action)
    디스패치를 통해 리듀서 함수를 실행하고, 파라미터로 전달된 액션을 분석해서
    스테이트를 업데이트한다.
*/

/*
Recucer 함수 : 스테이트를 업데이트하는 역할의 함수로 Redux의 Store와 유사하다.
               매개변수의 첫번째는 현재상태, 두번째는 상태변경을 위한 객체를
               전달받는다.
*/
