...
useState() : 리액트에서 상태값을 가지는 state의 값을 변경하거나 초깃값을
             부여할때 사용한다. 이 함수의 반환값은 배열인데 0번 요소는 
             state의 초깃값을 저장하는 변수이고 1번 요소는 이 값을 변경하는
             함수로 사용한다.
             구조분해할당을 이용해서 좌측항의 배열로 각각 값을 할당한다.
 */
function App() {
  /* 컴포넌트의 상태 관리를 위한 state생성. state의 변수명은 mode,
  초깃값은 both로 설정. 이를 변겅하기 위한 함수는 setMode()로 정의한다. */ 
  const [mode, setMode] = useState('both');

  // 컴포넌트 저장을 위한 변수 선언
  let contents = '';
  if(mode==='front'){
    // mode값이 front면 MyCont1 컴포넌트만 변수에 저장 후 렌더링한다.
    contents = <>
