...
function WriteForm(props){
  return(
    ...
    <form onSubmit={(e)=>{
      ...
      // 부모 컴포넌트에서 프롭스로 전달해준 함수를 호출하여 폼값을 전달한다.
      props.writeAction(writer,title,contents);
    }}>
...
function App() {
  ...
  return(<>
    <h2>React - Form값처리</h2>
    {/* 작성폼 컴포넌트를 추가하면서 프롭스를 통해 폼값을 받아 콘솔에 출력하는
    함수를 전달한다. */}
    ...
    {/* 스테이트가 변경되면 새롭게 리렌더링이 되므로 텍스트가 변경되는 것을 
    볼 수 있다. 즉 폼값 검증이 완료되었을때 변경된다. */}
    <p>{message}</p>
  </>)
}
export default App